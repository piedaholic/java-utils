...
public class DateUtils {
  ...
  /** Adds the to date. */
  /*
   * Add Day/Month/Year to a Date add() is used to add values to a Calendar object. You specify
   * which Calendar field is to be affected by the operation (Calendar.YEAR, Calendar.MONTH,
   * Calendar.DATE).
   */
  ...
  /*
   * Substract Day/Month/Year to a Date roll() is used to substract values to a Calendar object. You
   * specify which Calendar field is to be affected by the operation (Calendar.YEAR, Calendar.MONTH,
   * Calendar.DATE).
   *
   * Note: To substract, simply use a negative argument. roll() does the same thing except you
   * specify if you want to roll up (add 1) or roll down (substract 1) to the specified Calendar
   * field. The operation only affects the specified field while add() adjusts other Calendar
   * fields. See the following example, roll() makes january rolls to december in the same year
   * while add() substract the YEAR field for the correct result
   *
   */
  ...
  */
   ...
   *
   * @param date the date
   * @param noOfMonths the no of months
   * @return the java.sql. date
   */
  ...
  /*
   * public static boolean isLeapYear(int inYear) { boolean leapFlag = false; if (inYear % 4 == 0) {
   * if (inYear % 100 == 0) { if (inYear % 400 == 0) { leapFlag = true; } else { leapFlag = false; }
   * } else { leapFlag = true; } } else { leapFlag = false; }
   *
   * return leapFlag; }
   */
   ...
   * Format.
   *
   * @param pDateToFormat the date to format
  ...
  /**
   * Gets the zone date time.
   *
   * @param strTargetZone the str target zone
   * @return the zone date time
   */
  public static String getZoneDateTime(String strTargetZone) {

    java.util.Date date =
        convertToTimeZone(new java.util.Date(), ZoneId.systemDefault().getId(), strTargetZone);
    return PLSQL_FORMAT.format(date);
  }

  /**
   * Gets the util zone date time.
   *
   * @param strTargetZone the str target zone
   * @return the util zone date time
   */
  ...
  /**
   * Gets the util system date time.
   *
   * @param strSourceZone the str source zone
   * @param zoneDate the zone date
   * @return the util system date time
   */
  public static java.util.Date getUtilSystemDateTime(
      String strSourceZone, java.util.Date zoneDate) {
   ...
   *
   * @param date the date
   * @param tzFrom the tz from
   * @param tzTo the tz to
  ...
  public static java.util.Date convertToTimeZone(java.util.Date date, String tzFrom, String tzTo) {
    ...
    java.util.Date convertedDate =
        java.util.Date.from(
            LocalDateTime.ofInstant(date.toInstant(), ZoneId.of(tzTo))
                .atZone(ZoneId.of(tzFrom))
                .toInstant());
